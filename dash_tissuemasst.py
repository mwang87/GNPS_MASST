...
dash_app = dash.Dash(
    name="dashinterface",
    server=app,
    url_base_pathname="/tissuemasst/",
    external_stylesheets=[dbc.themes.BOOTSTRAP],
)

dash_app.title = 'tissueMASST'
...
NAVBAR = dbc.Navbar(
    children=[
        ...
        dbc.Nav(
            [
                dbc.NavItem(dbc.NavLink("tissueMASST Dashboard - Version 2025.03.25", href="/tissuemasst")),
            ],
        navbar=True)
    ],
    color="light",
    ...
    dbc.CardBody(
        [
            ...
            dbc.Row([
                dbc.Col([
                    html.Div(
                        dbc.Button("Search tissueMASST by USI", color="warning", id="search_button_usi", n_clicks=0),
                        className="d-grid gap-2",
                    )
                ]),
                dbc.Col([
                    html.Div(
                        dbc.Button("Search tissueMASST by Spectrum Peaks", color="warning", id="search_button_peaks", n_clicks=0),
                        className="d-grid gap-2",
                    )
                ]),
                dbc.Col([
...
def create_example(lib_id):
    return f"/tissuemasst#%7B%22usi1%22%3A%20%22mzspec%3AGNPS%3AGNPS-LIBRARY%3Aaccession%3A{lib_id}%22%2C%20%22peaks%22%3A%20%5B%22%22%5D%2C%20%22precursor_mz%22%3A%20%5B%22%22%5D%7D"
...
def draw_output(
    ...
    # TODO - tissueMASST maybe needs change
...
def draw_url(usi1, peaks, precursor_mz):
    ...
    return [request.host_url + "/tissuemasst#" + url_params]
