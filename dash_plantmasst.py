...
NAVBAR = dbc.Navbar(
    children=[
        ...
        dbc.Nav(
            [
                dbc.NavItem(dbc.NavLink("plantMASST Dashboard - Version 2025.02.24", href="/plantmasst")),
            ],
        navbar=True)
    ],
    color="light",
...
END IONS\n""".format(precursor_mz, peaks.replace(",", " ").replace("\t", " "))
    ...
    response_list = [html.Iframe(src="/plantmasst/results?task={}".format(mangling), width="100%", height="900px")]

    # Creating download link for the results
    response_list.append(html.Br())
    response_list.append(html.A("Download Results", href="/plantmasst/results?task={}".format(mangling), download="mangling.html", target="_blank"))
...
def plant_masst_path(task):
    ...
    html_file = os.path.join(output_folder, "fastMASST_plant.html")
